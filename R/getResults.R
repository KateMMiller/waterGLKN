...
#'
#' @param months Numeric. Months to query by number. Accepted values range from 1:12. #+++++ CHECK THIS Note +++++
#' that most of the events are between months 5 and 10, and these are set as the defaults.
#'
...
#'
#' @param output Specify if you want all fields returned (output = "verbose") or just the most important
#' fields (output = "short"; default.)
#'
...
getResults <- function(park = "all", site = "all", site_type = "all",
  ...
  sample_type <- match.arg(sample_type, several.ok, c("all", "VS", "QC", "rep"))
  ...
  parameter <- match.arg(parameter, several.ok = T, params)
