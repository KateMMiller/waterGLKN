...
<details><summary class = 'drop'>Troubleshooting Build Tools</summary>
...
<b>First install the `usethis` package if you don't have it installed. Then set .Renviron to find RTools on opening by opening the .Renviron file (second line), and pasting the code in the remaining lines into that file, and saving it. Now close/reopen RStudio, and you should be all set.</b>
```{r eval = F}
...
```
<details><summary class = 'drop'>Troubleshooting GitHub package installation</summary>
...
<span style='color:#1942B5;'><b>Option 1.</b> Import latest Lakes data package by specifying the folder where the unzipped csvs liv.</span
```{r eval = F}
...
print_head <- function(df){
  nrows <- min(6, nrow(df))
  knitr::kable(df[1:nrows,]) |> #, table.attr = "style='width:60%;'") |> 
    kableExtra::kable_classic(full_width = F, font_size = 12, 
                              bootstrap_options = c("condensed"))
}
